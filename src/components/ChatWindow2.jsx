...
const ChatSidebar = () => {
  ...
  const [userWithChatRoomWithParticipants, setUserWithChatRoomWithParticipants] = useState([]);
  ...
  useEffect(() => {
    ...
    socket.on("receive_message", (message) => {
      console.log("📨 Sidebar: Received message for room:", message.roomId);
      ...
      // Refresh messages query to get updated data
  ...
  useEffect(() => {
    if (data) {
      ...
      const chatRoomWithParticipants = chatRoomData.map((chat) => {
        ...
        console.log("final chat room id with participants :", { chatId, participants });
        return { chatId, participants };
      });

      setUserWithChatRoomWithParticipants(chatRoomWithParticipants);
      console.log("#############chatRoomWithParticipants ###########", chatRoomWithParticipants);
  ...
  // Create a comprehensive list of all chat rooms with their latest messages
  const getAllChatsWithMessages = () => {
    const chatRoomsWithMessages = [];

    // First, get all chat rooms from userWithChatRoomWithParticipants
    userWithChatRoomWithParticipants.forEach(chatRoom => {
      ...
      const messagesForThisRoom = allMessagesData.filter(msg => 
        msg.chatId && msg.chatId.roomId === chatId
      );
      ...
      if (socketUpdate && (!lastMessageTime || new Date(socketUpdate.createdAt) > new Date(lastMessageTime))) {
        latestMessage = {
          ...
          <div className="divide-y divide-gray-200">
            {filteredChats.map((chat) => (
              <ChatItem
                key={chat.chatId}
                chat={chat}
                isSelected={selectedChat?.chatId?.roomId === chat.chatId || selectedChat?.chatId?._id === chat.chatId}
...
const ChatItem = ({ chat, isSelected, onClick, otherParticipant, isOnline, isConnected }) => {
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between">
            ...
            {/* TODO : You can add unread message count here */}
          </div>
        </div>
      </div>
    </div>
